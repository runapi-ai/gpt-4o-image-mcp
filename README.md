<h1 align="center">GPT-4o Image MCP Server</h1>

<p align="center">
  <strong>Create GPT-4o Image AI tasks through RunAPI — 1 endpoint, 1 model — then poll status and check pricing. One MCP server, one API key.</strong>
</p>
...
`@runapi.ai/gpt-4o-image-mcp` is a focused MCP server for the **GPT-4o Image** model line on RunAPI.
It lets an MCP-compatible assistant create GPT-4o Image tasks, poll their status, and check current pricing — all through a single RunAPI API key.
...
| Tool | Auth | Purpose |
|---|---|---|
| `text_to_image` | Yes | Create a GPT-4o Image task (text to image) and optionally poll until it reaches a terminal status. Returns the task id, status, output URLs, and a price snapshot. |
| `get_task` | Yes | Fetch the current status and latest payload for an existing task. |
| `check_pricing` | No | Look up the current price for a GPT-4o Image model and endpoint. |

---

## Models

GPT-4o Image covers 1 model across 1 endpoint. Each tool accepts the models listed for it:
