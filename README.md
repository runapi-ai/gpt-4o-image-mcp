...
RunAPI MCP server for the **GPT-4o Image** model line. Create tasks,
poll their status, and check pricing through a single RunAPI API key.

## Tools

- `text_to_image` — create a GPT-4o Image task (text to image) and (optionally) poll until it reaches a terminal status. Returns the task id, status, output URLs, and a price snapshot. Models: `gpt-4o-image`.
...
```bash
npx -y @runapi.ai/gpt-4o-image-mcp
```
